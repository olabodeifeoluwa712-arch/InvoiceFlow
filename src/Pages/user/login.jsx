...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    if (res?.ok) {
      ...
      navigate(destination);
       ...
       return triggerError(res?.error || 'Login failed. Please try again.');
    }
    } catch (err) {
    //  console.log('error', res?.error)
     if(err instanceof ApiError) return triggerError(err || 'Login failed. Please try again.');
    }
  ...
  return (
    <div className="min-h-screen bg-[#EEEEF3] dark:bg-cyber-dark text-slate-800 dark:text-slate-100 flex flex-col font-sans transition-colors duration-300 select-none">
      ...
      {/* ── Top Navigation ── */}
      <header className="bg-[#EEEEF3] dark:bg-cyber-dark border-b py-3 px-6 md:px-10 flex justify-between items-center border-none">
      ...
      {/* ── Main Content ── */}
    <div className="min-h-screen bg-slate-100 dark:bg-[#0B0D10] flex items-center justify-center p-4 transition-colors duration-200 select-none">
      
      {/* ── Theme Toggle Pill ── */}
      <div className="fixed top-5 right-5 z-50">
        <button
          onClick={toggleTheme}
          className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-white dark:bg-[#171B21] border border-slate-200 dark:border-[#272D35] shadow-sm text-xs font-semibold text-slate-700 dark:text-[#F3F4F6] hover:bg-slate-50 dark:hover:bg-[#1D2229] transition-all cursor-pointer"
        >
          {theme === 'dark' ? (
            <>
      ...
      <div className="w-full max-w-4xl bg-white dark:bg-[#171B21] rounded-3xl shadow-xl shadow-slate-300/40 dark:shadow-none overflow-hidden border border-slate-200/60 dark:border-[#272D35]">
        <div className={`grid grid-cols-1 lg:grid-cols-2 min-h-[580px] ${animateShake ? 'animate-shake' : ''}`}>
          ...
          <div className="bg-gradient-to-br from-[#7F22FE] via-[#6f1ee8] to-[#5248cc] p-8 md:p-10 flex flex-col text-white relative overflow-hidden">
            ...
            <div className="flex-1 relative z-10 flex flex-col justify-center mb-4 min-h-[160px]">
              <div className="relative rounded-2xl overflow-hidden shadow-2xl shadow-black/40 border border-white/10">
                ...
                />
                <div className="absolute bottom-3 left-3 bg-white dark:bg-[#1D2229] rounded-xl px-3 py-2 shadow-lg flex items-center gap-2 border border-slate-100 dark:border-[#272D35]">
                  <div className="w-6 h-6 rounded-lg bg-emerald-100 dark:bg-emerald-950/40 flex items-center justify-center flex-shrink-0">
                    <svg className="w-3.5 h-3.5 text-emerald-600 dark:text-emerald-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d="M13 7h8m0 0v8m0-8l-8 8-4-4-6 6" />
                    </svg>
                  </div>
                  <div>
                    <p className="text-[9px] font-semibold text-slate-500 dark:text-[#A1A7B0] leading-none mb-0.5">Revenue this month</p>
                    <p className="text-sm font-extrabold text-emerald-600 dark:text-emerald-400 leading-none">+18.4%</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* ── RIGHT: Sign-in Form ── */}
          <div className="bg-white dark:bg-[#171B21] p-8 md:p-10 flex flex-col justify-center relative">

            <div className="inline-flex items-center self-start gap-1.5 px-3.5 py-1 rounded-full bg-violet-50 dark:bg-[#8B7CF6]/10 border border-violet-200 dark:border-[#8B7CF6]/25 text-[#7F22FE] dark:text-[#8B7CF6] text-[11px] font-bold tracking-wide mb-5">
              <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <h2 className="text-2xl font-extrabold text-slate-900 dark:text-[#F3F4F6] mb-1">Welcome Back</h2>
            <p className="text-sm text-slate-400 dark:text-[#A1A7B0] mb-6">Sign in to your InvoiceFlow account</p>

            {error && (
              <div className="mb-4 border border-red-300/50 bg-red-50 dark:bg-rose-950/30 dark:border-rose-500/25 text-red-600 dark:text-rose-400 px-4 py-2.5 rounded-xl text-sm flex items-center gap-2.5 animate-fade-in">
            ...
            {success && (
              <div className="mb-4 border border-emerald-300/50 bg-emerald-50 dark:bg-emerald-950/30 dark:border-emerald-500/25 text-emerald-600 dark:text-emerald-400 px-4 py-2.5 rounded-xl text-sm flex items-center gap-2.5 animate-fade-in">
                <svg className="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <form onSubmit={handleLogin} className="space-y-4">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-600 dark:text-[#A1A7B0] mb-1.5">Email Address</label>
                <div className="relative group">
                  <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400 dark:text-[#6F7782] group-focus-within:text-[#7F22FE] dark:group-focus-within:text-[#8B7CF6] transition-colors pointer-events-none">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-[#0F1216] border border-slate-200 dark:border-[#272D35] rounded-xl pl-10 pr-4 py-2.5 text-sm text-slate-800 dark:text-[#F3F4F6] placeholder-slate-400 dark:placeholder-[#6F7782] focus:outline-none focus:border-[#7F22FE] dark:focus:border-[#8B7CF6] focus:ring-2 focus:ring-[#7F22FE]/10 dark:focus:ring-[#8B7CF6]/20 transition-all font-medium"
                  />
                </div>
              </div>

              <div>
                <label className="block text-xs font-bold text-slate-600 dark:text-[#A1A7B0] mb-1.5">Password</label>
                <div className="relative group">
                  <span className="absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-400 dark:text-[#6F7782] group-focus-within:text-[#7F22FE] dark:group-focus-within:text-[#8B7CF6] transition-colors pointer-events-none">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <input
                    ...
                    className="w-full bg-slate-50 dark:bg-[#0F1216] border border-slate-200 dark:border-[#272D35] rounded-xl pl-10 pr-10 py-2.5 text-sm text-slate-800 dark:text-[#F3F4F6] placeholder-slate-400 dark:placeholder-[#6F7782] focus:outline-none focus:border-[#7F22FE] dark:focus:border-[#8B7CF6] focus:ring-2 focus:ring-[#7F22FE]/10 dark:focus:ring-[#8B7CF6]/20 transition-all font-medium"
                  />
                  <button
                    type="button"
                    onClick={() => setShowPassword(p => !p)}
                    tabIndex={-1}
                    className="absolute inset-y-0 right-0 flex items-center pr-3.5 text-slate-400 hover:text-slate-600 dark:text-[#6F7782] dark:hover:text-[#F3F4F6] transition-colors cursor-pointer"
                  >
                    {showPassword ? (
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              <div className="flex items-center justify-between">
                <label className="flex items-center gap-2 cursor-pointer">
                  <div
                    ...
                    className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all duration-200 cursor-pointer ${rememberMe ? 'bg-[#7F22FE] border-[#7F22FE] dark:bg-[#8B7CF6] dark:border-[#8B7CF6]' : 'bg-white dark:bg-[#0F1216] border-slate-300 dark:border-[#272D35]'}`}
                  >
                    {rememberMe && (
                  ...
                  <span className="text-xs font-medium text-slate-600 dark:text-[#A1A7B0]">Remember me</span>
                </label>
                <a href="#" className="text-xs font-semibold text-[#7F22FE] dark:text-[#8B7CF6] hover:text-[#6b1fd8] dark:hover:text-[#9B8CFF] transition-colors">
                  Forgot password?
                </a>
              </div>
